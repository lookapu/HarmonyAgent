...
/** 查询 Node 最新 LTS 版本号 */
export const fetchNodeLatestLts = () => invoke<string>('fetch_node_latest_lts')
...
export const installToolkit = (name: string, url: string, useProxy?: boolean) =>
  invoke<string>('install_toolkit', { name, url, useProxy: useProxy ?? null })
...
export const getToolVersion = (path: string) => invoke<string>('get_tool_version', { path })
