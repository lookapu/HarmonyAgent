...
export const getGitRuntime = () => invoke<GitRuntimeInfo>('get_git_runtime')

/** 查询 Git for Windows 最新发布 tag（GitHub API） */
export const fetchGitLatestVersion = () => invoke<string>('fetch_git_latest_version')

/** 升级 Git 运行时到最新版（下载 PortableGit 自解压包，静默解压生效）；useProxy: true=走系统代理 / false=直连 */
export const upgradeGitRuntime = (useProxy?: boolean) =>
  invoke<GitRuntimeInfo>('upgrade_git_runtime', { useProxy: useProxy ?? null })

/** 删除升级版，回到出厂捆绑版本 */
export const resetGitRuntime = () => invoke<GitRuntimeInfo>('reset_git_runtime')
