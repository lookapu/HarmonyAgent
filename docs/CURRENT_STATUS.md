...
| 输出退化（复读）防护 | 已实现 | `services/repetition.rs`：尾部窗口求最小周期 + 全文回扫确认，同一 ≥60 字节块连续重复 ≥6 次即判退化（阈值保守，正常长文/列表不误判，8 条用例含事故原文）；命中即掐断本轮、裁掉重复尾巴，并经 `KernelRoundControl::StopDegenerate` **只收尾不续写**（截断/中断分支会追加"请继续"，实测会把复读喂更长） | 判据是"连续重复"，模型换着花样复读（每遍改几个字）不覆盖；只对正文生效，不覆盖 reasoning；阈值取自事故数据的保守取值，真实长尾未统计 |
