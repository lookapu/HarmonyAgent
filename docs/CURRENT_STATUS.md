...
| 原生隔离与资源限制 | 部分 | `agent/native_limits.rs`（`RLIMIT_CPU` 实测生效）、`sandbox.rs` 原生后端、宿主直跑限额可用 `HARMONY_HOST_DIRECT_*` 显式开启 | **macOS 原生沙箱实际不可用**：本机实测「`deny default` + 路径限定允许」的 profile 连 `/usr/bin/true` 都 SIGABRT（普通 Terminal 亦复现），且能力探测用宽松 profile 会**假阳性报 available**（见盘点 §48，处置待定）；macOS 内核不接受 `RLIMIT_AS`（内存限不了）；进程数/CPU 配额/临时磁盘总量未限；Windows 生命周期未实现；OCI 端到端需 Docker |
