...
| 对话控制（停止 / 排队 / 回复语言） | 已实现 | 排队消息默认等当前任务收尾续跑（`take_next_queued`/`take_all_queued`），排队条另有「立即插入」：`stop_chat(resume_queued=true)` 置 `ChatResumeAfterStop`，任务在停止安全点不中断而是排空队列续跑，并 `clear_stop_requested` 清掉看门狗的停止计时（否则 40s 后被判 `stop_not_effective` 强杀续跑轮，已有用例钉住）；空闲态（停止后遗留的排队）改走普通发送并带回原 `references_json`；回复语言指令移到两份系统提示的**末尾**并加强措辞（`language_footer`） | 「立即插入」需真实模型跑一轮才能验证观感（单元/门禁只到标志位与清计时）；插入时被中断的那一轮按 `cancelled` 记一条 `task_runs`，等于一次用户动作两条记录；空闲态分支有几百毫秒的重入窗口，用 3s 防抖兜住（未做服务端幂等） |
