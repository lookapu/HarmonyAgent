...
| 对话控制（停止 / 排队 / 插入 / 回复语言） | 已实现 | 排队消息默认等当前任务收尾逐条续跑；排队条「立即插入」= `insert_queued_now` 置 `ChatForceMerge`，安全点**无条件**把排队消息并入**正在跑的任务**（不看 `agent_owned`、不停止、不新建轮次），当前任务照原目标继续——把"什么时候并"从模型自己决定改成用户点了就并；空闲态（停止后遗留的排队）改走普通发送并带回原 `references_json`；回复语言指令移到两份系统提示的**末尾**并加强措辞（`language_footer`） | 「立即插入」的消费点是轮边界（`assemble_round`），正在发出的那一轮模型调用不可中断，故最长等一轮（不是"等所有工具跑完"）；真实观感需跑一轮真实模型；`insert_queued_now` 自身的标志位消费依赖运行中的任务（无任务时标志留到下一次运行，行为等价于"下轮并入"） |
| 输出退化（复读）防护 | 已实现 | `services/repetition.rs`：尾部窗口求最小周期 + 全文回扫确认，同一 ≥60 字节块连续重复 ≥6 次即判退化（阈值保守，正常长文/列表不误判，8 条用例含事故原文）；命中即掐断本轮、裁掉重复尾巴，并经 `KernelRoundControl::StopDegenerate` **只收尾不续写**（截断/中断分支会追加"请继续"，实测会把复读喂更长）；警告注记只由轮级路由追加一次 | 判据是"连续重复"，模型换着花样复读（每遍改几个字）不覆盖；只对正文生效，不覆盖 reasoning；裁切点是"复读开始处"，若模型复读后又接了全新内容，那部分会一起被裁掉（复读场景下按收尾处理，已在注记里告知用户）；阈值取自事故数据的保守取值，真实长尾未统计 |
| 回复啰嗦（复述规则/自评合规） | 提示层已加约束 | 系统提示 core 增"输出纪律"：不复述或引用规则原文、不逐条自评合规（"✅ 已包含 4 个必备要素""本轮仅输出 X 未输出 Y"）、不在正文预告动作（"本轮将发起 2 个独立只读请求"）——要调工具就直接给【TOOL】标记；同时保留"任务总结须给出结论与真实证据" | 纯提示层约束，弱模型可能仍不听（本机事故那条回复 25% 是元叙述，计划正文夹在中间）；未加"检测元叙述占比并截断"的硬策略，因为"哪些算元叙述"没有可靠判据，误裁会切掉真内容 |
| 任务清单与对话的互动 | 已实现（双向） | 清单不再只写不读：`todo::render_hint` 每轮注入未完成项（≤12 条）并要求"完成一项就地标记 done"，未完成项存在时不得宣称完成；`todo_write` 一字未改时回"未变化，本次调用不构成进展"（原先照回"清单已更新"，模型据此以为有进展）；前端任务已结束但仍有 `in_progress` 项时不再转圈，如实标"未完成" | 未完成项**不硬拦**完成声明（只注入 + 提示），故仍可能出现"清单未完成而回复宣布完成"；清单由模型主动维护，只在它调 `todo_write` 时才更新 |
| 设备预览（驱动 SDK Previewer 取真实帧） | 已实现（双平台 CI 绿） | `services/previewer.rs`（定位引擎/组装参数/切帧/取帧客户端）+ `commands/preview.rs`（预览构建 → 起引擎 → 帧推给前端）+ 预览面板 + 面板组件测试；配方与实测见盘点 §52/§54；CI `35315010893` 双平台全绿（含本能力的单元与前端门禁） | 交互事件回传（点击/滑动送回引擎）、热重载、多设备档切换未做；**面板与后端的真实串联未在运行的应用里点过**——面板逻辑有组件测试（api 打桩）、后端链路由 e2e 覆盖，两者之间的**名字级**接线已由 `frontend_backend_contract` 钉住（命令名与事件名双向校验，见第 1 节），剩余未验的是 prop 传递与运行时的真实点击；引擎缺失时退回 Web 预览（显式报错，不静默失败）。**Web 预览与本机服务**：面板不再预填 `localhost:5173`（绿色版没有那个开发服务器，预填只会显示"拒绝连接"），并给内嵌 WebView 与主窗口加了 `--proxy-bypass-list=localhost;127.0.0.1;<local>`——WebView2 默认跟随系统代理，回环经代理失败时同样报"拒绝连接"，看起来像服务没起（本机实际反馈）；应用自身的 HTTP 客户端另有 `LOOPBACK_NO_PROXY`（`utils/net.rs`）。引擎端口由 `pick_port` 在 29000~50000 探测，占用/竞态下 WS 连不上会带端口号显式报错 |
