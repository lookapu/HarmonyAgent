...
| 长会话恢复与执行治理 | 基础实现 + 本机回归充分 | `agent/kernel_executor.rs` 检查点/安全点、预算继承、两组 crash E2E | 桌面 IO adapter 未统一（迁移方案见 [headless 驱动文档 §17](./HEADLESS_AGENT_DRIVER.md)，实测主循环体约 2,107 行，需先补行为快照测试）；真实长任务未验 |
