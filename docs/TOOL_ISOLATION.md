...
> 重要：本文的“隔离”是故障域和任务生命周期隔离，不是文件系统、网络或操作系统权限沙箱。普通 `run_command` 和兼容工具名 `sandbox_exec` 当前都以宿主用户权限启动子进程。`sandbox_exec` 仅把可选源目录复制到临时目录后试运行，不能阻止进程访问临时目录之外的宿主资源。完整边界见 [安全边界与威胁模型](SECURITY_BOUNDARY.md)。
