...
### 内部停止统一撤销 P5（2026-09-14）

工具 Worker 超时与取消轮询、看门狗强制终止、删除会话均接入 `stop_and_revoke`，补齐 P4 中内部停止路径缺口；现有桌面和 LAN 停止入口也使用该函数。停止信号先发出，再尝试获得数据库互斥锁；锁忙/损坏或 SQL 失败时返回明确错误，不撤销本地停止信号，不声称持久撤销完成。看门狗记录错误仍继续 abort；删除会话先 abort，再在撤销失败时拒绝继续删除数据。撤销表固定保留首个原因，重复停止不覆盖原始原因。

此处 `try_lock` 避免等待进程内数据库互斥锁，但不保证 SQLite 跨连接写锁无等待；数据库失败仍需排障与重试。现有 UI 停止按钮无需新增参数即可走统一实现，本轮未增加独立逐调用撤销按钮。仍不承诺撤销与外部进程派发/产物发布的原子性。

### 独立 OTA 审批撤销 P6（2026-09-14）

桌面 `revoke_ota_approval` 仅接受精确 tool-call ID，事务内要求对应 `ota_pack` 且处于 prepared/running/verifying/recovery_required/stuck，再幂等记录 `user_revoke_call`；不存在、已结束或其他工具被拒绝。此入口不发会话级停止信号，不影响其他调用，后续签发/执行/发布审批检查会发现持久撤销。

运行中的 OTA 工具卡提供撤销按钮和处理中/成功/错误状态；错误可重试，成功禁用按钮并保留说明。事件中的原始 callId 单独进入 ToolRun，不能把带 UI 前缀的 id 当作后端身份，缺少身份的历史记录不显示按钮。界面明确告知不能立即杀进程或撤回已发生副作用；状态为当前组件内反馈，不代表子进程已停止。本轮未增加 LAN 独立撤销 API，也未完成真实桌面与打包工具链端到端验收。
