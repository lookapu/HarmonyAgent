...
`deploy` 与 `deploy_all` 缺省不再递归扫描并按文件名猜“最新 HAP”，而是从最近 manifest 选择：

1. manifest 工程指纹必须与当前源码/配置一致；否则要求重新构建，显式 `hap` 可作为用户确认旧产物的覆盖方式；
2. 候选必须是本次 `build:` 来源的 HAP，模块和产品归属明确，文件仍位于工程根内；
3. 部署前重新计算 SHA-256，并重新检查 `.SF + 签名块`，两者都与 manifest 证据一致才可自动选择；
4. 可用 `product`、`module` 缩小范围；候选跨产品/模块，或最新时间点仍有多个文件时，工具返回候选路径、时间与短哈希并要求用户显式传 `hap`（或补齐 product + module）确认；
5. 只有唯一候选时按 `modified_at` 选择最新文件。显式 `hap` 仍拒绝明确 unsigned 文件，并在结果中回显重新计算的签名状态与短哈希。

自动选择是安装前门禁，设备侧 `hdc install` 与随后启动/存活检查仍是最终部署验收；manifest 不能替代设备证据。
