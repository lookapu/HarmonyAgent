...
- `SandboxBackend`/`SandboxSpec`、平台原生/可选 OCI capability 清单、fail-closed OCI argv、超时取消与审计事件、命令接线均已就绪；完整能力清单已注册为 Tauri 命令，探测不下载镜像、不执行用户命令。产品默认路线已改为无 Docker 依赖的平台原生轻量后端；原生执行 argv、文件边界和资源限制仍待实现，OCI 只保留为显式可选或外部 CI 适配器。
...
- [ ] 平台原生轻量 `SandboxBackend`；Shell/build/test 默认断网运行且不依赖 Docker（统一 `SandboxSpec`、后端契约、超时/取消、输出限制和审计事件已完成；macOS `sandbox-exec`、Linux bubblewrap、Windows AppContainer 的类型化 capability 与 Tauri 探测入口已落地，探测会运行最小隔离域且不下载镜像，未实现的 Windows 生命周期保持 unavailable；现有 Docker/Podman `OciBackend` 只作显式可选适配器，缺镜像/运行时会失败关闭。下一步实现原生执行 argv、工作区/网络边界和资源限制，再按平台逃逸套件结果决定默认启用）；
