...
- UI 证据链迁移：`take_screenshot`、`verify_ui` 和 `run_ui_flow` 的截图收尾已使用 Broker 的受管截图、工作区限定 recv 与设备临时文件清理能力；设备端仅允许 `/data/local/tmp/deveco_agent_` 下的安全 basename 和 `.png/.json/.mp4` 白名单后缀，本地 `.deveco-agent/screenshots` 在创建前后验证 canonical 工作区边界。自动探索与通用 UI 树工具的剩余直连待继续迁移。
...
- [x] 核心截图证据链通过 Broker 的固定 argv 生成、工作区限定拉取和受管临时文件清理执行；截图目录建立前后验证 canonical 边界，自动探索/UI 树剩余链路继续按同一模型迁移；
