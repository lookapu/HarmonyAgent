...
- Host Capability Broker：`HostCapability` 类型化窄能力覆盖 hdc 连接/断开/列表/daemon 生命周期、受限 pid/hilog/logcat、HAP install、ability 启动与 deploy；`connect_device`、`manage_hdc`、Agent 设备主清单、`read_logcat`，以及单设备 `deploy` 和 `deploy_all` 已通过固定 argv 执行入口。日志能力只接受类型化 device/bundle/level/tag/行预算，不暴露任意 shell。HAP canonicalize 后必须仍是工作区内普通文件，符号链接逃逸失败关闭；Broker 强制要求真实 run/tool-call 身份，幂等键同时绑定规范化目标以区分批量设备，设备目标在 started/finished/rejected 审计中只保留摘要。独立 SQLite claim 台账已把非 replay-safe 能力的 claim/started 与 finish/finished 分别原子提交，并受 Worker 租约 fencing 保护；重复派发失败关闭，进程中断的未完成 claim 转为 `indeterminate` 并强制 `verify_effects`。只读列表/日志允许安全重放，但每次仍校验租约并审计。该契约是最多一次派发，不宣称外部 exactly-once。设备富化查询、其他故障取证日志、模拟器、文件传输、受限 shell、签名/发布、自身影响/审批字段仍待迁移（`agent::capability_broker`）。
...
- [ ] Host Capability Broker 原型，先覆盖 `hdc` 与 deploy（类型化窄能力、安全校验、固定 argv 执行与 Durable Run 事件已落地；`connect_device`、`manage_hdc`、Agent 设备主清单、`read_logcat`、单设备 `deploy` 和 `deploy_all` 已接入，日志参数被限制为 device/bundle/level/tag/行预算。HAP canonicalize 后必须位于工作区，符号链接逃逸失败关闭；真实 run/tool-call 身份和稳定幂等键已进入 Broker 事件，设备目标仅以摘要审计。跨进程原子 claim、租约 fencing、重复派发拒绝和崩溃后 `indeterminate/verify_effects` 恢复已完成，replay-safe 只读查询允许重复但强制审计；设备富化查询、其他故障取证日志、模拟器、文件传输、受限 shell、签名/发布、自身影响/审批字段和真机验证待完成）；
