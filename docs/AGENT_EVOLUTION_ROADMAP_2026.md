...
- [ ] Host Capability Broker 原型，先覆盖 `hdc` 与 deploy（类型化窄能力、安全校验、固定 argv 执行与 Durable Run 事件已落地；`connect_device` 连接/断开、单设备 `deploy` 和 `deploy_all` 的 HAP install/ability start 已接入。HAP canonicalize 后必须位于工作区，符号链接逃逸失败关闭；真实 run/tool-call 身份和稳定幂等键已进入 Broker 事件，设备目标仅以摘要审计。跨进程原子 claim、租约 fencing、重复派发拒绝和崩溃后 `indeterminate/verify_effects` 恢复已完成；查询/日志、模拟器、签名/发布、自身影响/审批字段和真机验证待完成）；
