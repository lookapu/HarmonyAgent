...
- [ ] Tree-sitter/ArkTS 容错 AST 层与依赖/影响图（AST、`contains`、语法级 `extends/implements`、同文件唯一目标、相对命名 import、根 `tsconfig` path alias 及 HarmonyOS `file:/link:` 本地包入口查询时绑定已完成；re-export 与 calls 待完成）；
