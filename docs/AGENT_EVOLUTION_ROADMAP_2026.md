...
- Agent Kernel IO 收敛新增 Phase AG/AH：`KernelIoRunLoop` 已同时包裹 UI/headless executor，统一持有单调时钟并在 Provider 边界采样 elapsed；headless 工具剩余 wall time 也使用同一时钟。原始 `with_limits`/`begin_round(elapsed)` 已收为私有，生产 adapter 不能绕过循环壳。当前仍保留 adapter 的 Provider/工具/事件/DB 实现，下一阶段继续迁移这些异步端口，不把循环壳冒充完整单一 run-loop。
