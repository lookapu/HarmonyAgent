...
- Agent Kernel IO 收敛新增 Phase AG—AJ：`KernelIoRunLoop` 已同时包裹 UI/headless executor，统一持有单调时钟并在 Provider 边界采样 elapsed；headless 工具剩余 wall time 也使用同一时钟。原始 `with_limits`/`begin_round(elapsed)` 已收为私有，生产 adapter 不能绕过循环壳；新增 `KernelIoPort` 与异步 `run` 唯一实现取消采样、安全点、轮次驱动和终态退出。活跃 `KernelExecutorCheckpoint` schema v1 完整保存 router/governor/计数/首因，恢复时计入停机墙钟且未知版本失败关闭；adapter messages/工具结果仍需组合持久化。当前 UI/headless 的 Provider/工具/事件/DB 实现仍待迁为该端口，不把协议落地冒充生产迁移完成。
