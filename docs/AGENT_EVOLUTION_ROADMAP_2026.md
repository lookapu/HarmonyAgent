...
- [ ] Host Capability Broker 原型，先覆盖 `hdc` 与 deploy（类型化窄能力、安全校验、固定 argv 执行与 Durable Run 事件已落地；`connect_device`、`manage_hdc`、Agent 设备主清单、`read_logcat`、工作区限定的 `device_file`、`stop_app`、单设备 `deploy` 和 `deploy_all` 已接入。日志参数被限制为 device/bundle/level/tag/行预算；文件传输拒绝工作区外路径、`..`、符号链接逃逸和设备端相对路径。HAP canonicalize 后必须位于工作区；真实 run/tool-call 身份和稳定幂等键已进入 Broker 事件，设备/远端路径仅以摘要审计。跨进程原子 claim、租约 fencing、重复派发拒绝和崩溃后 `indeterminate/verify_effects` 恢复已完成，replay-safe 只读查询允许重复但强制审计；设备富化查询、其他故障取证路径、模拟器、通用受限 shell、签名/发布、自身影响/审批字段和真机验证待完成）；
