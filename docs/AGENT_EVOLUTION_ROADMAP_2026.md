...
- Host Capability Broker：`HostCapability` 类型化窄能力覆盖 hdc 连接/断开/列表/daemon 生命周期、受限 pid/hilog/logcat/faultlog 枚举、工作区限定的文件 send/recv、应用 force-stop、bundle uninstall、HAP install、ability 启动与 deploy；`connect_device`、`manage_hdc`、Agent 设备主清单、`read_logcat`、`search_hilog`、`device_file`、`stop_app`、`analyze_crash`、`device_shell`、`check_signature`、`dump_battery`、`stack_dump`、`set_network_condition`，以及单设备 `deploy` 和 `deploy_all`（含型号/安装状态/存活查询、失败日志取证与新装失败卸载补偿）已通过固定 argv 执行入口。日志能力只接受类型化 device/bundle/level/tag/有界尾部窗口/表达式摘要，faultlog 只允许三个枚举目录且过滤不安全 basename，不暴露任意 shell；文件传输拒绝工作区外绝对路径、`..`、本地父目录符号链接逃逸和设备端相对路径，崩溃副本固定写入工作区并在建目录前后验证 canonical 边界。HAP canonicalize 后必须仍是工作区内普通文件。Broker 强制要求真实 run/tool-call 身份，幂等键同时绑定规范化目标以区分批量设备/文件，设备与远端路径在审计中只保留摘要。独立 SQLite claim 台账已把非 replay-safe 能力的 claim/started 与 finish/finished 分别原子提交，并受 Worker 租约 fencing 保护；重复派发失败关闭，进程中断的未完成 claim 转为 `indeterminate` 并强制 `verify_effects`。只读列表/日志允许安全重放，但每次仍校验租约并审计。该契约是最多一次派发，不宣称外部 exactly-once。Agent 工具目录内的直接 `run_hdc_shell` 与裸 `run_cmd("hdc")` 已清零；Agent 设备清单的型号/API/ABI/屏幕/能力富化、默认设备选择以及 `device_perf` 采样也已全程保留 Broker 身份，默认选择只执行一次 targets 查询。模拟器进程本身、签名/发布、自身影响/审批字段仍待迁移（`agent::capability_broker`）。
