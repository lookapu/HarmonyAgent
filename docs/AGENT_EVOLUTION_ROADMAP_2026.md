...
- UI 证据链迁移：`take_screenshot`、`verify_ui`、`run_ui_flow` 的截图/断言收尾、`auto_explore` 和 `dump_ui_hierarchy` 已使用 Broker 的受管截图/UI 树、工作区限定 recv 与设备临时文件清理能力；设备端仅允许 `/data/local/tmp/deveco_agent_` 下的安全 basename 和 `.png/.json/.mp4` 白名单后缀，本地 `.deveco-agent/screenshots|explore|ui` 在创建前后验证 canonical 工作区边界。UI 输入动作和 locator 的剩余直连待继续迁移。
