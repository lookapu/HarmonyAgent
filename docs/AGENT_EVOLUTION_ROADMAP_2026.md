...
- `SandboxBackend`/`SandboxSpec`、平台原生/可选 OCI capability 清单、fail-closed argv、超时取消与审计事件、命令接线均已就绪；完整能力清单已注册为 Tauri 命令，探测不下载镜像、不执行用户命令。显式 `HARMONY_SANDBOX_BACKEND=native` 已将 `run_command` 接入 macOS `sandbox-exec`/Linux bubblewrap，使用干净环境、定向工作区、独立临时目录和默认断网；配置拼写错误、域名 allowlist、Windows 未实现路径以及隔离模式后台任务均失败关闭。产品默认路线保持无 Docker 依赖；完整资源限制、跨平台逃逸门禁和默认启用仍待完成，OCI 只保留为显式可选或外部 CI 适配器。
