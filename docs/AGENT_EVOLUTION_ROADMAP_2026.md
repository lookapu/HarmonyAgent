...
当前能力状态以 [当前状态单页](./CURRENT_STATUS.md) 为准；逐批过程与发现见 [主线实现与验收盘点](./MAINLINE_AUDIT_2026-09-14.md)。恢复 OTA 卡片身份、持久撤销展示、UI 实际审计查询、Broker 工作区身份、20 工具常驻预算与原生沙箱采集侧输出预算已落实。Windows 原生生命周期、系统资源总量限制、桌面 IO port、Java 类型语义（已接入单文件 javac 差分诊断，JDT LS 语义联动与 Maven/Gradle 全工程验证仍缺）和签名发布仍是代码待办，不归类为仅需真实验证。
...
Java 结构句柄补充边界检查：行范围必须与独占整行的完整 AST 声明吻合，包含相邻节点、多变量字段或遗漏附着注解时拒绝；单句柄、批量句柄、dry-run 同口径，UI 显示独立的边界拒绝提示。当前索引和句柄仍以行范围为主，尚未升级为跨语言字节级节点编辑，不能宣称任意格式的 Java 结构都可安全直接修改。

后续升级：Java **单句柄**在完整文件版本与身份校验后，从绑定行范围唯一定位同种 AST 声明，按原文 UTF-8 字节区间替换；同行不同种声明/注释不再被一并删除。多候选、同组多字段及缺少类型证据仍拒绝；**批量句柄**继续要求独占完整行。单节点 dry-run 已与写入共用候选语法门禁。此能力不等同于全语言字节句柄或类型语义分析。

最新补充：Java 同文件批量句柄也已接入字节区间候选拼接，全部定位成功且无重复/父子重叠后才验证与提交，报告仍按用户参数顺序。`edit_file` 写入助手增加提交前文件类型、长度及完整内容基线复验，拦截候选分析期间的外部改写；该前置检查不提供跨进程文件系统 CAS 保证。

多文件提交补充：`multi_edit` 增加整批提交前复验及逐文件写入前复验；回滚仅恢复仍匹配本事务候选的文件，已恢复原文可幂等跳过，外部新内容保留并列入未恢复项。恢复失败不再被吞掉；UI 与错误信封明确提示人工核验，回滚未完成不能被 busy/timeout 等瞬态关键词重新标成可自动重试。
...
- Host Capability Broker：`HostCapability` 类型化窄能力覆盖 hdc 连接/断开/列表/daemon 生命周期、受限 pid/hilog/logcat/faultlog 枚举、工作区限定的文件 send/recv、应用 force-stop、bundle uninstall、HAP install、ability 启动、设备 debuggerd attach/control、模拟器查询/生命周期、工作区 OTA 打包与 deploy；`connect_device`、`manage_hdc`、Agent 设备主清单、`read_logcat`、`search_hilog`、`device_file`、`stop_app`、`analyze_crash`、`device_shell`、`check_signature`、`dump_battery`、`stack_dump`、`set_network_condition`、`attach_debugger`、`step_debug`、`list_emulators`、`start_emulator`、`create_emulator`、`ota_pack`，以及单设备 `deploy` 和 `deploy_all`（含型号/安装状态/存活查询、失败日志取证与新装失败卸载补偿）已通过固定 argv 执行入口。日志能力只接受类型化 device/bundle/level/tag/有界尾部窗口/表达式摘要，faultlog 只允许三个枚举目录且过滤不安全 basename，不暴露任意 shell；文件传输拒绝工作区外绝对路径、`..`、本地父目录符号链接逃逸和设备端相对路径，崩溃副本固定写入工作区并在建目录前后验证 canonical 边界。HAP canonicalize 后必须仍是工作区内普通文件。Broker 强制要求真实 run/tool-call 身份，幂等键同时绑定规范化目标以区分批量设备/文件，设备与远端路径在审计中只保留摘要。独立 SQLite claim 台账已把非 replay-safe 能力的 claim/started 与 finish/finished 分别原子提交，并受 Worker 租约 fencing 保护；重复派发失败关闭，进程中断的未完成 claim 转为 `indeterminate` 并强制 `verify_effects`。只读列表/日志允许安全重放，但每次仍校验租约并审计。该契约是最多一次派发，不宣称外部 exactly-once。Agent 工具目录内的裸 HDC 与 Emulator 进程执行已清零；Agent 设备清单的型号/API/ABI/屏幕/能力富化、默认设备选择以及 `device_perf` 采样也已全程保留 Broker 身份，默认选择只执行一次 targets 查询。调试器 PID 与动作已分别收窄为正整数和封闭枚举；attach 超时等不确定终态失败关闭，不自动叠加 Ability 调试回退。模拟器可执行路径只由 Broker 从 DevEco 安装目录发现，实例/配置字段有界；创建/删除后读回实例清单，启动采用原子 claim 后派发并用 HDC 上线证据验证。尚未注册实现的独立签名/证书导入/市场发布能力，以及 Broker 自身影响/审批字段仍待完成（`agent::capability_broker`）。
- UI 证据与输入链迁移：`take_screenshot`、`verify_ui`、`run_ui_flow` 的截图/断言收尾、`auto_explore`、`dump_ui_hierarchy` 与现场 `ui_locator` 已使用 Broker 的受管截图/UI 树、工作区限定 recv 与设备临时文件清理能力；设备端仅允许 `/data/local/tmp/deveco_agent_` 下的安全 basename 和 `.png/.json/.mp4/.csv` 白名单后缀，本地 `.deveco-agent/screenshots|explore|ui|ui_records` 在创建前后验证 canonical 工作区边界。`record_ui` 的 start/stop、CSV 拉取/清理也已分解为窄能力，按设备原子登记进行中录制并要求 stop 名称一致，杜绝并发覆盖和错目标写入。`run_ui_flow`、自动探索导航、`replay_ui` 与 `gesture_perform` 的点击/滑动/长按/文本/按键也已改为类型化非重放能力，最终执行层二次校验坐标、速度、文本、键名和每次 operation id，并对输入文本只记录摘要。`collect_perf` 与 `run_perf_benchmark` 的系统 CPU/内存/温度、进程 pid/top 采样均已通过 replay-safe 查询能力执行；后者的 force-stop/start、Ability 状态、电量、RenderService FPS 与后台键，以及 `start_ability` 的明确 bundle+ability 和 URI/省略 ability 兼容启动均已复用对应窄能力。兼容启动只把 URI 作为独立 argv 传递，请求身份与审计仅保存 URI 摘要。`dump_memory`/`memory_snapshot`、`get_installed_apps` 与 `get_app_info` 的 pid、proc、hidumper、bm 查询同样通过只读能力执行，且内存快照会正确透传 device/bundle。`clear_app_data`、`uninstall_app`、`grant_permission` 与 Wi-Fi/飞行模式兼容后端也已类型化为非重放能力；卸载使用独立 bm dump 读回，无法明确确认“不存在”时失败关闭。`screen_record` 已使用专用有界长任务入口：spawn 前原子 claim，后台退出写终态，停止、工作区拉取和受管临时文件清理分别审计；进程崩溃遗留 claim 进入 `indeterminate/verify_effects`，不会自动重放。
...
- `SandboxBackend`/`SandboxSpec`、平台原生/可选 OCI capability 清单、fail-closed argv、超时取消与审计事件、命令接线均已就绪；完整能力清单和限定范围的原生文件边界 smoke 已注册为 Tauri 命令。显式 `HARMONY_SANDBOX_BACKEND=native` 已将 `run_command` 接入 macOS `sandbox-exec`/Linux bubblewrap，使用干净环境、定向工作区、独立临时目录和默认断网；配置拼写错误、域名 allowlist、Windows 未实现路径以及隔离模式后台任务均失败关闭。资源 capability 已拆分为墙钟、输出、CPU、内存、PID 和临时盘六项；原生后端只声明实际强制的墙钟/输出限制，UI 明示其余缺口。产品默认路线保持无 Docker 依赖；smoke 不覆盖网络、凭据、进程树与资源限制，完整资源限制、跨平台逃逸门禁和默认启用仍待完成，OCI 只保留为显式可选或外部 CI 适配器。
- `run_command` 的宿主、原生与 OCI 成功路径已共用同一文件变化扫描和 `record_cmd_changes` 收尾，隔离执行不会因提前返回而丢失间接修改证据；内部临时目录完全由 UUID 命名，外部 execution id 不参与路径构造。
...
- Host Capability Broker 的独立签名/证书导入/市场发布实现及已接线路径的真机/模拟器端到端验证。
...
退出门槛：安全边界已可通过 `harmony-agent sandbox verify --json` 单命令输出并以退出码门禁；检索性能和 25 题 Agent 基线继续使用各自的固定命令与产物契约复现。

### 2026-09-13：Host Capability 结果证据补强

- [x] 调试 PID 单值严格解析，拒绝多进程歧义、夹带文本和溢出；数字入参不再回退默认进程。
- [x] 模拟器创建/删除前置状态检查；启动前基线失败关闭；派发后查询失败报告未知状态；仅将已授权在线设备作为候选证据，不把新增设备等同于目标实例。
- [x] OTA 改为独占暂存、验证本次产物、无覆盖发布；保留稳定请求摘要，拒绝旧产物、悬空链接、空文件和并发覆盖，清理限于自有已知文件及空目录。
...
### 2026-09-13：OTA 内容摘要与能力作用域 P1

- [x] 审批/执行共享有效工作区根与暂存路径解析，避免相对路径解析漂移。
- [x] 审批展示前绑定 HAP/profile 内容摘要、canonical 输入/输出/工作区；工具入口和 Broker 实际能力参数分别复验，旧 v1 凭据要求重新审批。
- [x] 64 KiB 流式摘要、HAP 2 GiB/profile 16 MiB 上限、读取中变更检测及协作式时间预算；不持有数据库锁，不阻塞异步执行线程。
- 验证：新增 6 个作用域/凭据升级测试；Rust 库全量 1008 项，999 通过、0 失败、9 忽略，`git diff --check` 通过。
- 边界：这是执行前摘要复验，不是不可变内容副本；最终检查到子进程读取之间的并发替换风险、审批撤销及真实工具链端到端验收仍待完成。不使用 Docker。

### OTA 独立只读输入副本 P2

- [x] Broker 从已审批输入建立独占副本，复制摘要必须与审批一致；刷盘并设为只读后，Java 仅读取副本。
- [x] 固定 argv 只重定向输入，随机副本名不影响请求幂等键；审计记录输入副本策略。
- [x] 部分失败及正常返回清理自有已知副本，不递归删除未知内容；原文件不被修改，已有副本目标不覆盖。
- 验证（2026-09-14 恢复中断后复跑）：新增 5 个副本生命周期/参数绑定测试，包含未知文件保留和并发独立副本互不干扰；Rust 库全量 1013 项，1004 通过、0 失败、9 忽略，`git diff --check` 通过。
- 边界：只读独立副本不等于操作系统级不可变隔离；同权限恶意改写、Windows 私有 ACL、审批撤销和真实工具链验收仍待完成，崩溃残留需核验。不使用 Docker。

### 2026-09-14：OTA 审批生命周期 P3

- [x] v3 凭据绑定应用进程代次、会话停止代次及 30 分钟有效期；旧版本、重启、停止、过期均要求重新审批。
- [x] 审批等待前捕获停止基线，等待期间停止则拒绝签发；工具入口、Broker claim 和最终产物发布前均复验生命周期。
- 验证：Rust 库全量 1015 项，1006 通过、0 失败、9 忽略；新增停止代次、重启、过期及异常时间边界测试。
- 边界：本轮复用“停止任务”入口，不提供独立逐调用撤销 UI；不能撤回已经发生的外部副作用，跨进程停止广播及检查与发布之间的原子协调仍待完善。

### 2026-09-14：OTA 持久撤销 P4

- [x] 迁移 083 增加按工具调用固定的撤销记录；桌面停止会话/工具和 LAN 停止会话入口共用持久撤销。
- [x] 凭据签发、执行校验与发布校验均拒绝被撤销调用；恢复或迟到批准不能重新授权同一调用，新调用可重新审批。
- [x] 新增双数据库连接可见性、重复撤销、同调用恢复拒绝和新调用隔离测试。
- 验证：Rust 库全量 1017 项，1008 通过、0 失败、9 忽略；`git diff --check` 通过。
- 边界：数据库撤销仅覆盖显式停止入口，不等于所有看门狗/超时路径的跨进程广播；检查与执行/发布之间的原子协调、独立逐调用撤销 UI 仍待完成。

### 2026-09-14：内部停止路径统一撤销 P5

- [x] 工具 Worker 超时、取消轮询、看门狗和删除会话接入统一停止/撤销入口，显式停止与 LAN 路径保持复用。
- [x] 先发送本地停止，数据库互斥锁忙时立即返回持久撤销失败；看门狗记录失败并继续 abort，超时/取消返回中附带撤销错误。
- [x] 撤销记录保留第一次原因（用户、超时、看门狗、删除）；新增缺表失败、数据库锁占用、本地信号保留及原因幂等测试。
- 验证：Rust 库全量 1020 项，1011 通过、0 失败、9 忽略，`git diff --check` 通过；生产代码中原始停止信号调用已收敛到统一入口。
- 边界：数据库失败时不宣称跨进程撤销成功；SQLite 自身写锁仍受连接超时策略影响，独立撤销 UI、派发/发布原子协调和真实工具链验收仍待完成。

### 2026-09-14：独立 OTA 审批撤销 UI P6

- [x] 桌面 OTA 运行工具卡提供独立撤销按钮、处理中/成功/失败重试状态及中英文提示；不停止同会话其他工具。
- [x] 显式传递后端 callId，不把 UI 列表 ID 用作授权目标；缺少真实 ID 的历史卡片不提供撤销。
- [x] 后端事务检查目标工具与非终态状态，再写入精确调用撤销记录；拒绝不存在、非 OTA 和已结束目标，重复撤销幂等。
- 验证：Rust 库 1021 项，1012 通过、0 失败、9 忽略；前端工具卡新增成功/失败/缺少调用身份测试。
- 边界：本轮入口为桌面工具卡，不包含独立 LAN 撤销 API；撤销在后续审批检查生效，不立即杀进程，不撤回已发生操作，仍需真实桌面端到端验收。
...
- [ ] Host Capability Broker 原型，先覆盖 `hdc` 与 deploy（类型化窄能力、安全校验、固定 argv 执行与 Durable Run 事件已落地；`connect_device`、`manage_hdc`、Agent 设备主清单及富化、日志/崩溃/文件/应用/UI/性能/网络/调试工具、模拟器查询与生命周期、工作区限定 OTA 打包、单设备 `deploy` 和 `deploy_all` 的状态探测、故障取证与卸载补偿已接入，Agent 工具目录内直接 HDC/Emulator 进程调用已清零。日志参数被限制为 device/bundle/level/tag/有界尾部窗口/表达式；faultlog 枚举只接受三个固定目录并拒绝不安全 basename；文件传输拒绝工作区外路径、`..`、符号链接逃逸和设备端相对路径，崩溃副本固定写入工作区内。HAP canonicalize 后必须位于工作区；真实 run/tool-call 身份和稳定幂等键已进入 Broker 事件，设备/远端路径仅以摘要审计。跨进程原子 claim、租约 fencing、重复派发拒绝和崩溃后 `indeterminate/verify_effects` 恢复已完成，replay-safe 只读查询允许重复但强制审计；弱网 qdisc 配置使用有界非重放能力、单次 replace、读回与失败清除补偿；视频录制使用有界长任务 claim/终态协议；模拟器创建/删除后读回实例清单，启动在原子 claim 后后台派发并读取 HDC 上线证据。独立签名/证书导入/市场发布能力、自身影响/审批字段和真机验证待完成）；
- [x] 截图、自动探索和通用 UI 树证据链通过 Broker 的固定 argv 生成、工作区限定拉取和受管临时文件清理执行；所有证据目录建立前后验证 canonical 边界；
