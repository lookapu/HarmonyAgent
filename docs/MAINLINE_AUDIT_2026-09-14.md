...
## 58. 把「前端 ↔ 后端接口名」钉成契约（2026-09-18，`3358754`）

第 58 批：设备预览的面板接线此前只有静态检查——前端用**字符串**调 IPC、用**字符串**订阅事件，编译器两侧都管不到，名字写错时类型检查、前端组件测试（api 被打桩）与 Rust 测试**全都会通过**，直到有人在运行的应用里点那一下。补上这条缝的自动校验，顺带把同类风险从「只在预览这一处」扩到全仓。

- **新契约测试** `src-tauri/tests/frontend_backend_contract.rs`：① 前端 `invoke*('<name>'` 调到的每个命令必须在 `lib.rs` 的 `generate_handler!` 里注册——判据是 `::<name>,` 的包含关系，**不解析 handler 块**（块内含 `]` 的写法会把块截断，最初的探针就踩过这个坑，误报了 13 个「未注册」）；② 前端 `listen*('<event>'` 订阅的每个事件名必须在 Rust 源码里作为字面量出现。它跑在既有的 `cargo test --locked` 里，双平台 CI 自动覆盖，**不需要改工作流**。
- **两个防呆**：抽取下限断言（命令 ≥100、事件 ≥10）——抽取规则失效时必须**失败**而不是空过；模式串收紧为「函数名后紧跟泛型或括号」，因为宽松版把 `if (!listening) { window.addEventListener('keydown'` 与 `config.listen_address … {t('proxy.usage2')}` 这类跨段文本误判成了调用（这正是下限断言第一次跑就暴露的）。
- **实证非空过**：把前端 `'preview-frame'` 改成 `'preview_frame'`，事件那条立刻失败；还原后通过。
- **现状（0 不一致）**：命令 307 个全部已注册、事件 55 个全部存在于 Rust 侧；含本批新增的 `preview_start` / `preview_stop` 与 `preview-frame` / `preview-error`。
