...
所有工具出口（包括 MCP 成功与失败）、工具审计、`ToolResultV2`、可恢复人工交互、结构化应用日志、会话分享、快照和问题复现包统一使用 `utils::redact`。长输出先脱敏再写入产物，避免完整日志绕过上下文遮罩。
...
5. 问题复现包在统一规则后额外遮罩工程根与常见用户目录；附件先做项目边界、文件类型和敏感材料拒绝，再脱敏落盘。

复现包的预览绑定、附件拒绝、ZIP 完整性和确认规则见 [问题复现包规范 v1](REPRODUCTION_BUNDLES.md)。
