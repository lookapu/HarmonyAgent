...
同日增加了不创建海量实体文件的 SQLite 结构图基准，用来把“文件系统遍历/语法解析”和“持久图容量/查询”分开测量。最终 1M 档包含 1,000,000 条文件目录、1,250,000 个结构节点和 250,000 条 `contains` 关系：开发态单事务冷写入约 27.0 s，数据库约 572 MiB，精确名称定位约 1 ms，单节点邻接关系约 5 ms；按 `kind` 浏览到 90% 位置的第 18,000 页（每页 50 条）约 703 ms。对应查询已增加名称索引、`kind/file/line/name` 覆盖排序索引和执行计划回归测试；关系总量改为事务触发器维护的 O(1) 计数，避免每页扫描全边表。
...
1. 为结构浏览增加稳定游标/keyset 分页，消除深页 `OFFSET` 的线性扫描，并保留现有页码接口作为兼容层。
