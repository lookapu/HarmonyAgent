...
deferred 渐进解析现已接入：首次结构查询快速返回基础 4,000 文件后，每个仓库最多启动一个后台 worker，以 128 文件为一批从 SQLite 领取任务；源码读取和解析发生在事务外，提交时再次核对 path、size、mtime 和 `state='deferred'`，避免覆盖 watcher 已处理的外部变化。每批只提交对应节点与 `contains` 边，批间让出 20 ms；指纹漂移或数据库异常会停止任务并请求一致性扫描。后续全目录校验会保留指纹未变的后台成果，不会重新降级；进度直接反映在 indexed/deferred 覆盖统计中。进程退出时无需排空任务，SQLite 中的状态可在下次查询后继续推进。
...
1. 用渐进解析队列执行 1M 验收，加入 CPU/IO 背压、进度和取消观测，再按测量结果决定何时拆成物理数据库。
