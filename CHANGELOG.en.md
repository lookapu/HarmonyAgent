...
- API reference bodies are now resolved against the documentation catalog tree (`getCatalogTree`, 4,762 documents): coverage grew from 310 to **672 pages** (members 7,436 → 13,500). Documents whose title omits the module prefix (`@hms.ai.face.faceDetector` is titled just `faceDetector（人脸检测）`) are matched by a unique last segment plus parent-segment disambiguation; short or ambiguous segments are refused rather than guessed. Hardening: minimum segment length 4 → 3, parent-segment disambiguation when a segment collides (`@hms.core.map.map` picks `map-map`, not the Bluetooth MAP page), `errorcode-*` pages excluded, and ties broken **by namespace** (`@hms.*` gets the kit document, `@ohos.*` gets the `js-apis-*` one, so the two surfaces stop fighting over one row).
- Eight modules the rules cannot reach (e.g. `@hms.core.atomicserviceComponent.atomicservice`, `@kit.AREngine`) were checked page by page against the official body text and pinned in an explicit mapping table; the criteria are the kit a page imports, the `-api`/`-capi-` suffix, and whether the page's capabilities match the module's d.ts. Module-level misses are down to 34, all of them documents the catalog simply does not contain.
