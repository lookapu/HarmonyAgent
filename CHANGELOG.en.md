...
- API reference bodies are now resolved against the documentation catalog tree (`getCatalogTree`, 4,762 documents): coverage grew from 310 to **664 pages** (members 7,436 → 13,236). Documents whose title omits the module prefix (`@hms.ai.face.faceDetector` is titled just `faceDetector（人脸检测）`) are matched by a unique last segment; short or ambiguous segments are refused rather than guessed. Matching was hardened further: minimum segment length 4 → 3, parent-segment disambiguation when a segment collides (`@hms.core.map.map` picks `map-map`, not the Bluetooth MAP page), `errorcode-*` pages excluded, and `js-apis-*` preferred on ties — 27 more candidates resolved with no existing mapping changed.
