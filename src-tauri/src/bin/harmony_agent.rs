...
fn parse_args(args: impl IntoIterator<Item = String>) -> Result<EvalRunArgs, String> {
    ...
    while let Some(flag) = values.next() {
        let value = values.next().ok_or_else(|| format!("{flag} 缺少参数\n{}", usage()))?;
        match flag.as_str() {
            "--task" => task = Some(PathBuf::from(value)),
            "--workspace" => workspace = Some(PathBuf::from(value)),
            "--run-config" => run_config = Some(PathBuf::from(value)),
            "--driver" => driver = Some(PathBuf::from(value)),
...
fn execute(args: EvalRunArgs) -> Result<i32, String> {
    let task = parse_eval_task(&read_bounded(&args.task, "task")?)?;
    let config: EvalRunConfig = serde_json::from_str(&read_bounded(&args.run_config, "run config")?)
        .map_err(|e| format!("run config JSON 无法解析：{e}"))?;
    let workspace = args.workspace.canonicalize()
        .map_err(|e| format!("workspace 不存在或不可访问：{e}"))?;
    if !workspace.is_dir() { return Err("workspace 必须是目录".into()); }
    if args.output.exists() { return Err("output 已存在；为防覆盖，请指定新的输出目录".into()); }
    let driver = args.driver.canonicalize()
    ...
    match run_trial(&task, &workspace, &args.output, &adapter, &config) {
        Ok(outcome) => Ok(if outcome.status == OUTCOME_RESOLVED { 0 } else { 1 }),
...
fn main() {
    ...
    match execute(args) {
        ...
        Err(error) => { eprintln!("eval run 失败：{error}"); std::process::exit(2); }
    }
}
...
mod tests {
    ...
    fn parses_required_values_and_repeated_driver_args() {
        let parsed = parse_args(["eval", "run", "--task", "t.json", "--workspace", "repo",
            "--run-config", "r.json", "--driver", "/bin/agent", "--driver-arg", "--model",
            "--driver-arg", "x", "--output", "out"].into_iter().map(str::to_string)).unwrap();
    ...
    fn rejects_unknown_or_incomplete_commands() {
        ...
        assert!(parse_args(["eval", "run", "--wat", "x"].into_iter().map(str::to_string)).is_err());
    }

    #[cfg(unix)]
    #[test]
    fn execute_runs_a_complete_local_trial() {
        use std::os::unix::fs::PermissionsExt;
        let root = std::env::temp_dir().join(format!("harmony-eval-cli-{}", uuid::Uuid::new_v4()));
        let repo = root.join("repo");
        std::fs::create_dir_all(&repo).unwrap();
        std::fs::write(repo.join("a.txt"), "base\n").unwrap();
        for args in [["init", "-q"].as_slice(), ["add", "a.txt"].as_slice(),
            ["-c", "user.email=e@x", "-c", "user.name=t", "commit", "-q", "-m", "base"].as_slice()] {
        ...
        let code = execute(EvalRunArgs { task: task_path, workspace: repo, run_config: config_path,
            driver, driver_args: vec![], output: output.clone() }).unwrap();
        assert_eq!(code, 0);
