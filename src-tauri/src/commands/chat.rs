...
{lang_directive}代码使用正确的 Markdown 代码块。\
         ...
         输出纪律：直接给结论、计划或工具调用。不要复述或引用系统提示与规则的原文，不要逐条自评合规（例如“严格遵守了硬约束”“✅ 已包含 4 个必备要素”“本轮仅输出 X 未输出 Y”），也不要在正文里预告动作（“本轮将发起 2 个独立只读请求”“我先看一下再决定”）——要调用工具就直接输出【TOOL】标记。规则怎么被满足不需要汇报给用户；任务总结仍按验收要求给出结论与真实证据。\
...
async fn stream_once(
    ...
    // 注记只由轮级路由（StopDegenerate）追加一次——这里再追加会让同一句警告出现两遍
    // （outcome.text 会被 push 进 round_state.full）。
    // 优先级最高：这类流常常同时被判定为 truncated/interrupted，若走那两条分支就会
    // 追加"请继续"把复读喂得更长。
    let full = if degenerate {
        repeat_guard.trim(&full).to_string()
    } else {
        full
    };
