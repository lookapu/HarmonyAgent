...
/// 按 (仓库, 子目录, 作用域) 查技能
pub fn find_skill_by_repo(conn: &Connection, owner: &str, name: &str, subdir: &str, project_id: Option<&str>) -> Result<Option<Skill>, rusqlite::Error> {
    let mut stmt = conn.prepare(
        "SELECT id, name, description, directory, repo_owner, repo_name, repo_branch,
                subdir, enabled, content_hash, installed_at, updated_at, project_id
         FROM skills WHERE repo_owner = ?1 AND repo_name = ?2 AND IFNULL(subdir, '') = ?3
           AND (?4 IS NULL AND project_id IS NULL OR project_id = ?4)",
    )?;
    let mut rows = stmt.query_map(rusqlite::params![owner, name, subdir, project_id], |row| {
        Ok(Skill {
            id: row.get(0)?,
            name: row.get(1)?,
            description: row.get(2)?,
