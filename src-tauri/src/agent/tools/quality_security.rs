...
pub async fn obfuscate(args: &Value, roots: &[String]) -> Result<String, String> {
...
pub async fn sandbox_exec(args: &Value, roots: &[String]) -> Result<String, String> {
...
fn copy_tree(src: &Path, dst: &Path, depth: u32) -> Result<u32, String> {
    ...
    for e in std::fs::read_dir(src).map_err(|e| e.to_string())?.flatten() {
        ...
        if crate::agent::scanner::SKIP_DIRS.contains(&name.as_str()) {
            continue;
        }
