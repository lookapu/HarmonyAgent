...
//! NDJSON（覆盖面最广：早期 0.x 与 2025-06-18 之后均用此格式）；探测超时
//! 时不立即回退——超时多为 npx 冷下载/服务器未就绪，进程存活期间在同一管道
//! 混发 Content-Length 帧会打崩 SDK 1.x 的按行解析器（JSON.parse 收到
//! "Content-Length:" 头直接抛错，后续请求永久无响应）；仅当进程重启后仍
//! 无响应才切换 Content-Length 重探。接收时自动检测两种格式，并跳过通知帧（无 id）。
