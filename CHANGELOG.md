...
- 长会话加固批次（6 项）：预算按会话阶段动态画像分配（`balanced/explore/execute/verify`，读取时按 `agent_runs.phase` 推导）；85% 窗口压缩预警通知链路确认（`chat-context-warning` 事件 + 前端通知）；上下文面板新增分层预算条、画像标签、失效次数与会话健康度展示（压缩后自动刷新）；100+ 轮随机事件压力回归测试（压缩/失效/对账交错后事实裁决仍正确）；知识检索多 token 模糊召回（`search_knowledge_fuzzy`，命中数+字符重叠度排序）；会话健康度与摘要退化检测（压缩次数/事实翻转率/对账纠正次数/预算占用，`get_session_health` + 074 迁移）。详见 [长会话上下文 V2 设计](docs/CONTEXT_V2.md)。
