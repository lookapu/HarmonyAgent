...
定位：把“模型能调用很多工具”升级为“任务和工具都可持久调度、可验收、可恢复、可观测”。可靠性与治理批次新增迁移 `057`—`062`、`069`—`075`；当前继续推进长会话 Context V2，数据库迁移总数达到 **77**。治理批次共新增 3 个对外工具（`workflow_template`、`team_share`、`reproduction_bundle`），`TOOL_SPECS` 达到 **201**。
