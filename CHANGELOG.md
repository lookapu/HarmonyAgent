...
## Unreleased — 官方文档抓取链路重建、API 26 数据补齐与语义化版本号

**修复：华为文档站下线 `.md` 端点后，API 知识库整体失效**

- 版本 diff 与 API 参考正文原先依赖「页面 URL 追加 `.md` 返回 Markdown」的端点，华为已下线该端点（全站 404），两条链路实际空跑。现改用文档中心自身的 `documentPortal/getDocumentById` 接口取正文（HTML），新增 HTML 锚点/表格提取与 HTML→Markdown 转换。
- 版本清单入口改为按内容下钻：`apidiff-2600` 只列 release/beta 子入口（26.0.0 Release = `apidiff-7003`），逐层收集 Kit diff 页（最多 3 层）。修好后同一套流程也补全了历史版本此前漏抓的页面。
- API 参考兜底 slug 按线上实际 objectId 逐条校正（13 个 camelCase slug 已 404）。
- 修掉一个抓取回归：HTML→Markdown 的标签分支把相对偏移当绝对下标用，转换会陷入死循环。

**版本号：26.0.0 起为语义化版本**

- 官方从 API 26.0.0 起改用 `X.Y.Z`（取代 `X.Y.Z(N)`），次序为 `26.0.0 > 6.1.1(24) > … > 5.0.5(17)`。版本解析、比较与格式校验统一到一个实现，工程解析、SDK 对齐、API 可用性判断、新建工程的 `sdk_version` 校验与 `sdk-pkg.json` 组合全部走它。
- 提示词与内置知识条目同步：26.0.0 之前写 `6.1.1(24)`，26.0.0 起写 `26.0.0`；`26.0.0(26)` 属臆造组合会被拒。

**数据**

- API 参考正文改为按文档目录树精确查表（`getCatalogTree` 返回 4,762 篇）：覆盖由 310 页提升到 **664 页**（成员 7,436 → 13,236），`@hms.*` 这类页面标题不含模块前缀的文档改由「末段唯一匹配」命中；末段过短或撞名时宁可不匹配。后续加固：末段最短长度 4→3、撞名按父段消歧（`@hms.core.map.map` 取 `map-map` 而非蓝牙 MAP）、排除 `errorcode-*` 错误码页、平票优先 `js-apis-*` —— 这轮再补 27 个候选命中，且未改动任何既有映射。
...
- 种子库导入新增「同版本数据刷新」下发：版本号没变但条目变多时（26.0.0 Beta → Release），按种子批次修订号只增不删地补入，老用户不再停在旧口径。

**兼容与回滚**

- 无数据库迁移、无 schema 变化；种子库为本地生成物（不随仓库分发），可用 `cargo run --bin full_fetch --features full-fetch,embedding --release -- --seed` 重新生成（约 6 分钟：抓取 1 分钟 + 向量索引 4 分钟）。
- 旧的 `.md` 抓取代码路径已删除；若华为恢复该端点，回滚本次抓取改造即可。

## v2.2.0 — 结构优先导航、多语言写入门禁与可信执行内核（2026-09-15）

定位：把「能改代码」升级为「改得对、改得可验证」——写入前有语言级门禁，宿主操作有可撤销的审批凭据，资源与隔离有明确支持矩阵。本版跨 259 个提交。

**结构优先导航与大仓索引**

- 新增结构优先的代码导航与持久化仓库目录：全库文件登记、增量文件 delta、git checkpoint 对账、索引 watcher 与容错回退；百万文件规模下渐进索引有独立基准（见 [索引规模基线](docs/INDEX_SCALE_BASELINE.md)）。
- 结构查询支持稳定游标、分页结构节点与持久结构图边；查询结果与安全文件读取窗口绑定。

**写入门禁（本版重点）**

- 多语言真实门禁：Java（Tree-sitter 声明/注解检查 + `javac` 差分，含同源码根下**未参与编辑的调用方**）、Dart（`dart analyze` 差分）、Go（`go vet` 差分，临时模块并带上同包兄弟文件）、Python（`pyflakes` 差分）、SQL（`sqlite3` 内存库执行差分，含副作用语句保护）。
- 语法层覆盖 ArkTS/TS/JS/Go/Python/Rust/Dart/Kotlin/C++/Swift；外部工具缺席或超时一律降级为「未做该项检查」并记录事件，不阻塞写入、不冒充已校验。
- 多文件事务：整批与逐文件写入前复验基线、失败按逆序条件回滚、保留外部新修改并如实报告「回滚未完成」。

**宿主能力审批与可撤销凭据**

- 审批凭据 v4 与能力无关（OTA 的内容摘要只是其中一种作用域），撤销判定改为自描述规则；变更类宿主能力由 Agent 发起时必须持有有效凭据，停止会话或应用重启即失效。
- 审批卡片新增影响契约（可逆性 / 影响范围 / 目标 / 后果），同一套说法进入待确认项恢复与审计时间线。

**沙箱与资源限制**

- 原生沙箱对子进程施加可执行的 rlimit（CPU 时间实测生效；内存与进程数按平台如实标注不可用/未映射），宿主直跑可用环境变量显式开启限额。

**UI 与质量门禁**

- 设计 token 收敛到 IDE 质感（圆角/阴影/字号阶梯），静态内联样式改工具类。
- 文档漂移门禁与 clippy 告警基线门禁恢复全绿；新增[当前状态单页](docs/CURRENT_STATUS.md)作为能力状态的唯一口径。

## v2.1.1 — 交互与质量门禁加固（2026-08-25）

- Provider Token 输入框现在预填完整数值，并补充 K/M 缩写提示；回复语言与推理语言配置进一步完善。
- 文件拖拽支持外部文件、批量导入与浏览器回退路径，工程内引用统一保存为可移植相对路径；补齐 SVG MIME 识别、重复与失败反馈及相关回归测试。
- 完善中英文 README、开发与发布文档，修复文档漂移检查，并补齐 Node.js 版本预检与前端产物体积门禁。
- 修复 Windows/macOS 工具链与打包路径兼容性问题，收紧 ESLint、Rust 测试、Clippy 和跨平台发布前质量检查。

## v2.1.0 — 证据驱动治理与双层执行内核（2026-08-22）

定位：把“模型能调用很多工具”升级为“任务和工具都可持久调度、可验收、可恢复、可观测”。可靠性与治理批次新增迁移 `057`—`062`、`069`—`075`；当前继续推进长会话 Context V2，数据库迁移总数达到 **84**。后续能力叠加后，`TOOL_SPECS` 达到 **204**。

- 长会话加固批次（6 项）：预算按会话阶段动态画像分配（`balanced/explore/execute/verify`，读取时按 `agent_runs.phase` 推导）；85% 窗口压缩预警通知链路确认（`chat-context-warning` 事件 + 前端通知）；上下文面板新增分层预算条、画像标签、失效次 数与会话健康度展示（压缩后自动刷新）；100+ 轮随机事件压力回归测试（压缩/失效/对 账交错后事实裁决仍正确）；知识检索多 token 模糊召回（`search_knowledge_fuzzy`， 命中数+字符重叠度排序）；会话健康度与摘要退化检测（压缩次数/事实翻转率/对账纠正 次数/预算占用，`get_session_health` + 074 迁移）。详见 [长会话上下文 V2 设计](docs/CONTEXT_V2.md)。
- 长会话关联加强批次（5 项 + 评测 1 项，2026-08-22）：阶段分类器统一（`BudgetProfile::from_goal_or_phase`，运行态 phase 用 goal 兜底，动态预算真正生效）；上下文面板展示失效原因明细（“为什么遗忘”入口）；健康度退化判定改用摘要覆盖游标链推导真实层级（压缩次数仅作辅助）；压缩事件写入会话事件流（`context_compress`，Timeline 可回放）；核心指标表与评测快照补充退化指标（压缩/翻转/预警计数）；固定评测新增 100 轮长会话压缩恢复用例（27 个场景）并纳入基线门禁。
...
- 工具总数以 `src-tauri/src/agent/tools/mod.rs` 中 `TOOL_SPECS` 数组长度为准（当前 204）。
