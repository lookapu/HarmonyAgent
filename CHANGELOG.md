...
- 出厂种子库重新抓取：14 个版本 / 1,014 个页面 / 47,410 条变更，`api_docs` 46,700 → 91,556 行；其中 26.0.0 由 5,781 → 11,489 条（含 8 月 29 日 Release 新增的 `apidiff-7003` 一整套）。
- 种子库导入新增「同版本数据刷新」下发：版本号没变但条目变多时（26.0.0 Beta → Release），按种子批次修订号只增不删地补入，老用户不再停在旧口径。

**兼容与回滚**

- 无数据库迁移、无 schema 变化；种子库为本地生成物（不随仓库分发），需要回退时用同目录 `knowledge.db.bak`。
