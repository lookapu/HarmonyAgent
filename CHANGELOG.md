...
定位：把“模型能调用很多工具”升级为“任务和工具都可持久调度、可验收、可恢复、可观测”。可靠性与治理批次新增迁移 `057`—`062`、`069`—`073`；当前继续推进长会话 Context V2，数据库迁移总数达到 **73**。团队共享管理新增 1 个对外工具，`TOOL_SPECS` 达到 **200**。
...
- 工具总数以 `src-tauri/src/agent/tools/mod.rs` 中 `TOOL_SPECS` 数组长度为准（当前 200）。
